import { db, InvoiceStatus } from '@dealflow360/db';
import {
...
export class InvoiceService {
  ...
  async createInvoiceFromQuotation(
    ...
    // Idempotency / Duplicate Invoice Safeguard
    ...
    // Map quotation lines for authoritative calculation snapshot
    const lineInputs: InvoiceLineCalculationInput[] = quotation.lines.map((l) => ({
      ...
      unitPrice: l.unitPrice || l.listPrice, // Approved selling price!
    ...
    // Calculate default due date (14 days from issue)
    ...
    // Advance quotation status to BILLING if currently APPROVED or FULFILLMENT
  ...
  async markInvoicePaid(id: string, actor?: { id?: string; name?: string; role?: string } | null) {
    const invoice = await db.invoice.findUnique({ where: { id } });
    ...
    // Advance quotation status to COMPLETED
    await db.quotation.update({
