...
test('Visual Audit for Warehouse, Inventory, Kanban, and Backorders', async ({ page }) => {
  ...
  // Login via demo button or login form
  const demoRepBtn = page.locator('button:has-text("Sales Rep"), button:has-text("Sales Manager"), button:has-text("Admin")').first();
  if (await demoRepBtn.isVisible()) {
    console.log('Clicking demo login button...');
    await demoRepBtn.click();
  } else {
    console.log('Filling login form...');
    await page.fill('input[type="email"]', 'rep@dealflow.com');
